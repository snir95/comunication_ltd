...
function renderHome(req, res, user, messages = [], searchQueryResult = [], searchQuery = null) {
    res.render('home.ejs', {
    ...
    if (searchQuery) {
        ...
        db.clientDbConfig.query(searchQuerySQL, [searchTerm, searchTerm, searchTerm, searchTerm], (error, results) => {
            if (error) {
                ...
                return renderHome(req, res, req.session.user, req.flash('error'));
            }
            renderHome(req, res, req.session.user, [], results, searchQuery);
        });
    } else {
        renderHome(req, res, req.session.user);
    }
});

// Display dashboard
router.get('/', requireAuth, (req, res) => {
    renderHome(req, res, req.session.user, req.flash('error'));
});
...
router.post('/addClient', requireAuth, (req, res) => {
    ...
    if (!isValidphoneNumber(phone_number)) {
        ...
        return renderHome(req, res, req.session.user, messages);
    }
